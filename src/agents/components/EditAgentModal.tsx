...
import { uploadAgentAvatar, deleteAgentAvatar } from '../../utils/imageUpload';
      ...
      try {
        ...
        if (agentDoc.exists()) {
          ...
          setFormData({
            ...
            total_transactions: data.total_transactions?.toString() || '0',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (avatarFile) {
        ...
        if (currentAvatar) {
          await deleteAgentAvatar(currentAvatar);
        }

        const uploadResult = await uploadAgentAvatar(avatarFile, formData.code);
